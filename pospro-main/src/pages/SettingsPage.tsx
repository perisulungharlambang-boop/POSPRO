...
const SettingsPage: React.FC = () => {
  ...
  // ✅ SINKRONISASI & VERIFIKASI DATA — Berfungsi di SEMUA Platform (Browser, Android, Tauri)
  const handleSync = async () => {
    ...
    try {
      ...
      if (native && sqliteAvailable) {
        // ✅ ANDROID / iOS: Sync IndexedDB ↔ SQLite asli
        setSyncProgress({ step: 'Memulai sinkronisasi IndexedDB ↔ SQLite...', percent: 0 });

        const result = await dbProvider.syncDatabases((step, current, total) => {
          setSyncProgress({
            step,
        ...
        if (result.errors.length > 0) {
          ...
          showStatus(
            'error',
            `Sinkronisasi selesai dengan ${result.errors.length} error. Produk: ${result.products.idbToSql + result.products.sqlToIdb} disinkron, Transaksi: ${result.transactions.idbToSql + result.transactions.sqlToIdb} disinkron. Cek konsol untuk detail.`,
            6000
          );
        } else if (totalMoved === 0) {
          showStatus('success', '✅ Database sudah sinkron! Tidak ada perbedaan data antara IndexedDB dan SQLite.');
        } else {
          showStatus(
            'success',
            `✅ Sinkronisasi berhasil! Produk: ${result.products.idbToSql} IDB→SQL, ${result.products.sqlToIdb} SQL→IDB (${result.products.conflicts} konflik diselesaikan). Transaksi: ${result.transactions.idbToSql} IDB→SQL, ${result.transactions.sqlToIdb} SQL→IDB.`,
            6000
          );
        }
      } else {
        // ✅ BROWSER / TAURI: Verifikasi integritas data IndexedDB
        setSyncProgress({ step: 'Memverifikasi integritas data...', percent: 10 });
        ...
        setSyncProgress({ step: 'Memeriksa data produk...', percent: 40 });

        // ✅ Ambil sebagian produk untuk verifikasi
        const sampleProducts = await indexdbBarang.getPaged(0, 5);
        let productSummary = '';
        if (sampleProducts.length > 0) {
          // Cek apakah produk memiliki semua field yang diperlukan
        ...
        setSyncProgress({ step: 'Finalisasi...', percent: 90 });

        setTimeout(() => {
          setSyncProgress(null);
          const platform = tauri ? 'Tauri (Windows)' : 'Browser';
          showStatus(
            'success',
            `✅ [${platform}] Data terverifikasi!\n` +
            `📦 ${productCount} produk · 🧾 ${transactionCount} transaksi · 👥 ${customerCount} pelanggan · 🚚 ${supplierCount} supplier\n` +
            `${productSummary}\n` +
            `Semua data tersimpan aman di IndexedDB.`
          );
        }, 500);
      }
    } catch (e) {
        ...
        <div className="bg-white p-8 rounded-[32px] border border-slate-100 shadow-sm space-y-6 md:col-span-2">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">IndexedDB ↔ SQLite</p>
            </div>
          </div>
          ...
          <div className={cn(
            ...
            <div className="space-y-1">
              ...
              <p className={cn(
                ...
                {false && false
                  ? 'Kedua database aktif. Sinkronisasi akan menyamakan data antara IndexedDB (cache) dan SQLite (utama) menggunakan strategi last-write-wins berdasarkan timestamp.'
                  : 'Semua data tersimpan aman di IndexedDB. Tombol ini akan memverifikasi integritas data Anda — menampilkan jumlah produk, transaksi, pelanggan, dan supplier yang tersimpan.'}
              </p>
            </div>
          </div>
          ...
          {!syncProgress && (
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <div className="bg-slate-50 p-4 rounded-[20px] text-center space-y-2">
                ...
                <div className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Browser Cache</div>
                <div className="text-[10px] text-slate-400">barangDB + transaksiDB</div>
              </div>
              <div className="flex items-center justify-center">
                <div className="flex flex-col items-center gap-1">
                  <div className="text-indigo-400 font-black text-lg">⇄</div>
                  <div className="text-[9px] font-black text-indigo-400 uppercase tracking-widest">Last Write Wins</div>
                </div>
              </div>
              <div className="bg-slate-50 p-4 rounded-[20px] text-center space-y-2">
                <div className="text-lg font-black text-slate-600">SQLite</div>
                <div className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Native Primary</div>
                <div className="text-[10px] text-slate-400">poskasir.db</div>
              </div>
            </div>
          )}
