 ...
 * 
 * IndexedDB Service untuk Biaya/Pengeluaran (Expenses)
 * Menyimpan data biaya operasional toko
 */
...
class IndexDBExpense {
  ...
  private initDb(): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      request.onerror = (event) => {
        console.error("IndexedDB expenseDB error:", (event.target as IDBOpenDBRequest).error);
  ...
  private getObjectStore(mode: IDBTransactionMode): IDBObjectStore {
    if (!this.db) {
      throw new Error("Database expense not initialized.");
    }
  ...
  async save(expense: Expense): Promise<void> {
    return new Promise(async (resolve, reject) => {
      await this.initDb();
      if (!expense.id) {
        reject(new Error("Expense id is required"));
        return;
      }
